...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD,
  ...
  // 步骤3：构造Frame
  mCurrentFrame = Frame(mImGray, mImDepth, timestamp, mpORBextractorLeft,
                        mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
  // 步骤4：跟踪
  // cout << "frame construct finished " << endl;
  Track();
...
void Tracking::Track() {
  ...
  if (mState == NO_IMAGES_YET) {
    mState =
        NOT_INITIALIZED;  //判断系统是否初始化，没有初始化就跳转到初始化函数，这是单目所独有的过程；
  }
  ...
  } else  // 步骤2：跟踪
  {
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == OK) {
        ...
        if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
          ...
          if (!bOK)
            // 当使用运动模式匹配到的特征点数较少时，就会选用关键帧模式。即尝试和最近一个关键帧去做匹配。为了快速匹配，本文利用了bag
            // of
            // words（BoW）来加速。首先，计算当前帧的BoW，并设定初始位姿为上一帧的位姿；其次，根据位姿和BoW词典来寻找特征匹配，使用函数matcher.SearchByBoW()；最后，利用匹配的特征优化位姿。TrackReferenceKeyFrame是跟踪参考帧，不能根据固定运动速度模型预测当前帧的位姿态，通过bow加速匹配（SearchByBow）
            // 最后通过优化得到优化后的位姿
            bOK = TrackReferenceKeyFrame();
        }
      } else {
    ...
    // cout << "位姿估计成功，准备跟踪局部地图　" << endl;
    // 将最新的关键帧作为reference frame
    mCurrentFrame.mpReferenceKF = mpReferenceKF;

    // If we have an initial estimation of the camera pose and matching. Track
    // the local map. 步骤2.2：在帧间匹配得到初始的姿态后，现在对local
    // map进行跟踪得到更多的匹配，并优化当前位姿 local
    // map:当前帧、当前帧的MapPoints、当前关键帧与其它关键帧共视关系
    ...
    if (bOK) {
      ...
      // frame. 删除那些在bundle adjustment中检测为outlier的3D map点
      for (int i = 0; i < mCurrentFrame.N; i++) {
...
void Tracking::StereoInitialization() {
  if (mCurrentFrame.N > 500) {
    ...
    mpReferenceKF = pKFini;  //关键帧作为参考帧 (追踪线程成员)
    mCurrentFrame.mpReferenceKF =
        pKFini;  //关键帧作为参考帧？？？？？？？？？？？（当前关键帧成员）
...
void Tracking::UpdateLastFrame() {
  ...
  mLastFrame.SetPose(
      Tlr * pRef->GetPose());  // Tlr*Trw = Tlw 1:last r:reference w:world
...
bool Tracking::TrackWithMotionModel() {
  ...
  // 根据Const Velocity
  // Model(认为这两帧之间的相对运动和之前两帧间相对运动相同)估计当前帧的位姿，nani
...
bool Tracking::NeedNewKeyFrame() {
  ...
  // relocalisation 步骤2：判断是否距离上一次插入关键帧的时间太短
      ...
      for(int i =0; i<mCurrentFrame.N; i++)
      {
          if(mCurrentFrame.mvDepth[i]>0 && mCurrentFrame.mvDepth[i]<mThDepth)
          {
              ...
              if(mCurrentFrame.mvpMapPoints[i])
                  if(mCurrentFrame.mvpMapPoints[i]->Observations()>0)
                      nMap++;//
  被关键帧观测到的mappoints数，即观测到地图中的MapPoints数量
          }
      }
  }
  else
...
void Tracking::CreateNewKeyFrame() {
  ...
  mpPointCloudMapping->InsertKeyFrame(pKF, this->mImRGB, this->mImDepth, idk);
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  // check which keyframe shares most points V-D K1: shares the map points with
  // current frame 策略1：能观测到当前帧MapPoints的关键帧作为局部关键帧
  ...
  // already-included keyframes V-D K2: neighbors to K1 in the covisibility
  // graph 策略2：与策略1得到的局部关键帧共视程度很高的关键帧作为局部关键帧
  for (vector<KeyFrame *>::const_iterator itKF = mvpLocalKeyFrames.begin(),
                                          itEndKF = mvpLocalKeyFrames.end();
       itKF != itEndKF; itKF++) {
...
bool Tracking::Relocalization() {
  ...
  // relocalisation 步骤2：找到与当前帧相似的候选关键帧
  ...
  vector<vector<MapPoint *>>
      vvpMapPointMatches;  //关键帧（向量）的路标点（向量）
